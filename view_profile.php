...
if(isset($_GET['email'])){
  $em = $_GET['email'];
  $query = "
  SELECT * FROM profile WHERE email='$em';
  ";
  ...
  if(mysqli_num_rows($query_run)){
    while($row = mysqli_fetch_array($query_run)){
      ...
      $private_post = 0;
      $query1 = "
      SELECT * FROM diary WHERE author_email = '$em'
      ";
      ...
      $query1 = "
      SELECT id, title, dateAndTime, status FROM diary WHERE author_email = '$em' ORDER BY id DESC
      ";
      $query_run1 = mysqli_query($connection, $query1)or die("<script>msg('Opps! Something wrong','red')</script>");
      while($row = mysqli_fetch_array($query_run1)){
        $visi = $row['status'];
